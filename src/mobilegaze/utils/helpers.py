...
def draw_gaze_keypoints(frame, bbox, keypoints, pitch, yaw, color=(0, 0, 255), thickness=2):
    """Draws gaze direction on a frame given bounding box and gaze angles."""
    # Unpack bounding box coordinates
    x_min, y_min, x_max, y_max = map(int, bbox[:4])
    ...
    x_center = int((keypoints[0][0] + keypoints[1][0]) // 2)
    y_center = int((keypoints[0][1] + keypoints[1][1]) // 2)
...
def draw_keypoints(frame, keypoints, color=(0, 255, 0), thickness=2):
    for idx, keypoint in enumerate(keypoints):
        ...
        cv2.circle(frame, (x, y), radius=2, color=color, thickness=thickness)
...
def draw_bbox_keypoints_gaze(frame: np.ndarray, bbox, keypoints, pitch, yaw):
    draw_bbox(frame, bbox)
    draw_keypoints(frame, keypoints)
    # draw_gaze(frame, bbox, pitch, yaw)
    draw_gaze_keypoints(frame, bbox, keypoints, pitch, yaw)